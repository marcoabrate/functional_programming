...
// Evaluation -----------------------------------
  ...
  def eval1(x: Data, env: Environment): Data = x match {
    ...
    case 'case :: value :: cases =>
      def caseEval(casei:Any, other_cases:List[Any]): Data = (casei, other_cases) match {
        case ('else :: expr :: Nil, Nil) => eval(expr, env)
        case (eq :: expr :: Nil, otca) => if (eval(value, env) == eq) eval(expr, env) else caseEval(otca.head, otca.tail)
      }
      caseEval(cases.head, cases.tail)
    ...
    // evaluate something of the form (def (succ x) (+ x 1) (succ 0))
    case 'def :: (Symbol(name) :: args) :: body :: expr :: Nil =>
      val lambda_ = List('lambda, args, body)
      if (env == globalEnv)
        globalEnv = env.extendRec(name, env1 => eval(lambda_, env1))
      eval(expr, env.extendRec(name, env1 => eval(lambda_, env1)))
