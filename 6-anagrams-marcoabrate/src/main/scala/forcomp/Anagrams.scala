...
object Anagrams extends AnagramsInterface {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.toList.groupBy((c:Char) => c).mapValues(_.size).toList.sorted
   ...
   *
   *  Hint: you can use foldLeft, and -,
   *  apply and updated operations on Map.
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    (y foldLeft SortedMap[Char, Int]() ++ x)(
      (occ_map: Map[Char,Int], occ:(Char,Int)) =>
        // remove the elements with same number 
        if (occ._2 == occ_map(occ._1)) occ_map - occ._1
        else occ_map updated (occ._1, occ_map(occ._1) - occ._2)
    ).toList
   ...
   *  and how should the result of a recursive invocation be integrated in each iteration? Also, using
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    if (sentence.isEmpty) List(List())
      ...
      def occurrenceAnagrams(occs: Occurrences): List[Sentence] =
        if (occs.isEmpty) List(List())
        else
          for {
            occ <- combinations(occs)
            word <- dictionaryByOccurrences.getOrElse(occ, Nil)
            sentence <- occurrenceAnagrams(subtract(occs, occ))
            if !occ.isEmpty
          } yield word :: sentence
